...
int main(int argc, char *argv[]){
	...
	cerr << "\n\tTEST #12: Shortest Path on Map 3" << endl;
        ...
        path = {"lucie", "lexi", "colleen", "laura"};
        checkPath(map3, path, "lucie", "laura");

        cerr << "\n\t========================PASS========================\n" << endl;
	
	cerr << "\n\tTEST #13: Distance between two cities on Map 3" << endl;
        start = map3.findByName("lexi");
        stop = map3.findByName("jill");
        assert(map3.pathDistance(start, stop) == 32);

        cerr << "\n\t========================PASS========================\n" << endl;

	cerr << "\n\tTEST #14: If there's no path between 2 cities returns -1" << endl;
        start = map3.findByName("kendra");
        stop = map3.findByName("nightvale");
	unsigned int test = -1;
        assert(map3.pathDistance(start, stop) == test);
